...
static int is_a_nb_character(char c)
{
    if ('0' <= c && c <= '9')
        return 1;
    else
...
static int is_negative(char const *str, int mk1)
{
    if (str[mk1-1] == '-')
        return -1;
    else
...
static int power(int a, int pow)
{
    int result = 1;
...
static int return_nbr(char const *str, int mk1, int mk2)
{
    int n = mk2-mk1;
    int unsigned sum = 0;
    int power_count = n-1;
